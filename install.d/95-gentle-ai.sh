...
# Receipt-driven development (RDD): opt-in por defecto. Decisión del usuario:
# dejarlo horneado APAGADO en scope global (~/.claude) para que el review nativo
# NO corra en ningún workspace salvo que se active manualmente.
log "Desactivando receipt-driven development (scope global)…"
gentle-ai review mode disable --scope global >/dev/null 2>&1 || log "WARN: no pude desactivar RDD."
